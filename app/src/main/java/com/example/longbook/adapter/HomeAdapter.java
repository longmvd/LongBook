...
import com.example.longbook.ReplacerActivity;
import com.example.longbook.model.HomeModel;
...
public class HomeAdapter extends RecyclerView.Adapter<HomeAdapter.HomeHolder> {
    ...
    public void onBindViewHolder(@NonNull HomeHolder holder, int position) {
        try {
            ...
            Glide.with(context.getApplicationContext()).load(list.get(position).getImageUrl()).placeholder(new ColorDrawable(color)).timeout(7000).into(holder.imageView);

            holder.clickListener(position, list.get(position).getId(), list.get(position).getName(), list.get(position).getUid(), list.get(position).getLikes(), list.get(position).getImageUrl());
        } catch (Exception e) {
            e.printStackTrace();
        }
